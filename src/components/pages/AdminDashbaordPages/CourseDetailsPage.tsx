        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <Textarea
              ...
              className={`${isEditMode ? "bg-white" : "bg-gray-50"} min-h-[80px]`}
              readOnly={!isEditMode}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input value={course.category.name} className="bg-gray-50" readOnly />
            </div>
            <div>
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            <Button onClick={handleAddLesson} className="bg-green-600 hover:bg-green-700 text-white">
              Add Lesson
            </Button>
            <Button onClick={handleAddModule} className="bg-blue-600 hover:bg-blue-700 text-white">
              Add Module
            </Button>
          </div>
        </div>
        <Accordion type="single" collapsible className="space-y-2 pb-5">
              ...
              <AccordionContent className="px-4 pb-3">
                <div className="space-y-2">
                  {module.lessons.map((lesson, idx) => (
                    <div key={lesson.id} className="p-2 border rounded-md bg-gray-50">
                      <p className="text-gray-800 font-medium">
                        {index + 1}.{idx + 1} {lesson.title}
                      </p>
                      <p className="text-sm text-gray-500">
                        {lesson.type === "video" ? `🎥 ${lesson.duration}` : `📝 ${lesson.duration}`}
                      </p>
                    </div>
                  ))}
                  {module.Quiz.length > 0 && (
                    <div className="p-2 border rounded-md bg-yellow-50">Quiz Available: {module.Quiz.length}</div>
                  )}
                </div>
              </AccordionContent>
            </AccordionItem>
          ...
          <AccordionItem value="certification" className="border border-gray-200 rounded-lg">
            <AccordionTrigger className="px-4 py-3 text-left hover:no-underline cursor-pointer">
              <span className="text-gray-700 font-medium font-sans">Certification</span>
            </AccordionTrigger>
            <AccordionContent className="px-4 pb-3">
              <p className="text-gray-600">You’ll receive a certificate after completing all lessons and quizzes.</p>
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </div>
        ...
        {course.instructor ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input value={course.instructor.username} className="bg-gray-50" readOnly />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Email</label>
              <Input value={course.instructor.email} className="bg-gray-50" readOnly />
            </div>
          </div>
        ) : (
          <p className="text-gray-500">No instructor assigned for this course.</p>
        )}
      </div>
      ...
      <div className="mb-8">
        {isEditMode ? (
          <div className="flex gap-4">
            ...
            <Button onClick={handleCancelEdit} variant="outline" className="flex-1 py-3 rounded-lg bg-transparent">
              Cancel
            </Button>
          </div>
        ) : (
