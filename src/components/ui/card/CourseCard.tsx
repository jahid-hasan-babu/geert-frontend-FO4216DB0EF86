...
export default function CourseCard({
  ...
  const [addToFavorites, { isLoading: favLoading }] =
  ...
  const toggleFavorite = async (event: React.MouseEvent) => {
    ...
    try {
      ...
      // Trigger translation refresh just in case dynamic text changed
      ...
      >
        ...
        <div className="flex flex-col p-4 h-full space-y-4">
          <div className="flex text-2xl font-bold text-gray-900 font-playfairDisplay justify-between items-start">
            <div className="flex-1 truncate" data-translate>
              {course.title}
            </div>
            {course.isMicroLearning && (
              <div
                className="ml-2 mt-1 bg-[#3399CC] text-white py-1 px-2 rounded-full text-[10px] font-sans whitespace-nowrap self-start"
                data-translate
              >
                Microlearning
              </div>
