...
export const AddLessonModal: React.FC<AddLessonModalProps> = ({
  ...
  const handleSubmit = async (values: LessonFormData) => {
    try {
      ...
      // Ensure durationSecs is a number
      formData.append("durationSecs", (values.durationSecs || 0).toString());
      if (videoFile) {
  ...
  const handleDocumentUpload = (file: File): boolean => {
    const allowedTypes = [
      "application/pdf",
      "application/msword",
  ...
  const convertTimeToSeconds = (timeString: string): number => {
    ...
    if (minutes < 0 || seconds < 0 || seconds > 59) {
      toast.error("Invalid time values. Minutes and seconds must be valid.");
      return 0;
    }
              ...
              return (
                <div className="bg-blue-50 p-4 rounded-lg">
                  <p className="text-blue-700">
                    Quiz lessons will be created separately in the quiz management
                    section. This will create a placeholder lesson that can be
                    linked to a quiz later.
                  </p>
                </div>
              );
            }
