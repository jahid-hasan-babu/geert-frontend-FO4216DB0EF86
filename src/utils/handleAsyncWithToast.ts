/* eslint-disable @typescript-eslint/no-explicit-any */ import { toast } from "sonner";
export const handleAsyncWithToast = async (
  ...
  try {
    ...
    if (res?.data?.success) {
      toast.success(res.data.message || successMessage, { id: toastInit });
    }
    if (res?.message) {
      toast.success(res.message, { id: toastInit });
    }
    if (!res?.data?.success) {
      toast.error(res?.error?.data?.message, { id: toastInit });
    }
    return res;
  } catch (error) {
    toast.error(
